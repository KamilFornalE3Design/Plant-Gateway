...
namespace SMSgroup.Aveva.Application.CLI.PGedge.Convert
{
    ...
    {
        private readonly PipelineContract<TakeOverPointDTO> _pipeline;
        private readonly IPipelineCoordinator<TakeOverPointDTO> _coordinator;
        private readonly IPipelineDiagnoser<TakeOverPointDTO> _diagnoser;

        private readonly INotificationService _notifications;

        public TakeOverPoint(PipelineContract<TakeOverPointDTO> pipeline, IPipelineCoordinator<TakeOverPointDTO> coordinator, IPipelineDiagnoser<TakeOverPointDTO> diagnoser, INotificationService notifications)
        {
            _pipeline = pipeline;
        ...
        public override int Execute(CommandContext context, ConvertSettings settings)
        {
            try
            {
                ...
                _pipeline.Output = TargetBuilder.BuildOutput(settings, _pipeline.Input, typeof(TakeOverPointDTO));

                // ─────────────────────────────
                //  Step 1: Determine Phase
                // ─────────────────────────────
                var phase = ProcessPhaseExtensions.ParsePhase(settings.Phase);
                ...
                // ─────────────────────────────
                var result = _coordinator.Run(phase);
